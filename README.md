...
Promo = {
    ...
    "🎭Get Mon 🎭",
    "🎭Mon  Best Lock🎭",
    "🎭Mon  Best Da Hood Script🎭",
    "🎭Mon  On Git🎭",
    "🎭Mon  Keyless🎭",
    "🎭HMon t Best AutoStomp🎭",
    "🎭Mon  Best Animations🎭"
}
    ...
    local function onKeyPress(input, gameProcessed)
        if not gameProcessed then
            if _G.autoStomp and input.KeyCode == stompKey then
                ...
                if isLooping then
                    showNotification("Auto Stomp Enabled", "🎭  Mon  🎭", 3)
                    startStomping()  -- Now calling startStomping properly
                else
                    showNotification("Auto Stomp Disabled", "🎭  HMon  oit 🎭", 3)
                    stopStomping()
                end
            end
        end
    ...
    LeftGroupBox:AddToggle('MyToggle', {
        ...
        Callback = function(Value)
            ...
            if _G.autoStomp then
                ...
                showNotification("Auto Stomp Disabled", "🎭  Mon  🎭", 3)
            end
        end
    })
    ...
    local function teleportAndStomp(targetPlayer)
        if targetPlayer and targetPlayer.Character then
            ...
            if targetHumanoid and torso and playerRoot then
                ...
                showNotification("Stomping", "🎭  Mon 🎭", 3)
            ...
            showNotification("No Target Found", "🎭  Mon 🎭", 3)
        end
    end
        ...
        elseif Config.enabled and (Config.spyOnMyself == true or p ~= player) then
            ...
            if hidden and Config.enabled then
                if Config.public then
                    saymsg:FireServer((Config.publicItalics and "/me " or '') .. "{Mon} [" .. p.Name .. "]: " .. msg, "All")
                else
...
WatermarkConnection = game:GetService('RunService').RenderStepped:Connect(function()
    ...
    Library:SetWatermark(('Mon  | %s fps | %s ms'):format(
        math.floor(FPS),
        ping
    ))
end)
